...
def main():
	...
	for i in range(100000):
		...
		Z = softmax( net_kj )
		delta_w_kj = learningRate * (Y.T.dot(t-Z) + momentum*delta_w_kj_prev + reg*w_kj)
		delta_w_ji = learningRate * (X.T.dot( gradient_sigmoid(Y) * np.dot(t-Z, w_kj.T) ) + momentum*delta_w_ji_prev + reg*w_ji)
		w_kj += delta_w_kj
		w_ji += delta_w_ji
